...
> Присваивание переменной другой переменной с **неизменяемым** значением осуществлчется **по значению**. В первой переменной окажется дудликат значенич второй переменной

> Присваивание переменной другой переменной с **изменяемым** значением осуществяется **по ссылке**. В первой переменной окажется тот же объект что и во второй. При изменении значения в любой из переменных изменятся данные в объекте и соответственно в обоих переменных
...
while x!=0: # запускаем цикл заполнения стека. допустим при ваеденном нуле нужно остановиться
  A[top]=x 
  top += 1
  x=int(input(x))
